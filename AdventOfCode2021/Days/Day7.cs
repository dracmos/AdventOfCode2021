...
namespace AdventOfCode2021.Days
{
    public class Day7
    {
        ...
        public class Crab
        {
            ...
            public Dictionary<long, long> FuelFromHLocation { get; set; }
        }
        ...
        private void CalculateElements(bool isPartTwo)
        {
            var numbersCrabsLocationsDict = new Dictionary<long, long>();

            foreach (var crabLocation in startingInputList)
            {
                if (!numbersCrabsLocationsDict.ContainsKey(crabLocation))
                    numbersCrabsLocationsDict[crabLocation] = 1;
                else
                    numbersCrabsLocationsDict[crabLocation] += 1;
            }

            var crabsDict = new Dictionary<long, Crab>();

            foreach (var crabLocationItem in numbersCrabsLocationsDict)
            {
                var crab = new Crab { HLocation = crabLocationItem.Key, Occurences = crabLocationItem.Value, FuelFromHLocation = new Dictionary<long, long>() };
                var maxLocation = startingInputList.Max();
                for (int i = 0; i < maxLocation; i++)
                {
                    long fuel = Math.Abs(crabLocationItem.Key - i);

                    if (isPartTwo)
                    {
                        long numberOfSteps = fuel;
                        fuel = (numberOfSteps * (numberOfSteps + 1)) / ((long)2);
                    }

                    crab.FuelFromHLocation.Add(i, fuel * crabLocationItem.Value);
                }
                crabsDict.Add(crabLocationItem.Key, crab);
            }

            var sumFuelByHorizontal = new Dictionary<long, long>();

            foreach (var crab in crabsDict.Values)
            {
                foreach (var fuel in crab.FuelFromHLocation)
                {
                    if (!sumFuelByHorizontal.ContainsKey(fuel.Key))
                    {
                        sumFuelByHorizontal[fuel.Key] = fuel.Value;
                    }
                    else
                    {
                        var currentFuel = sumFuelByHorizontal[fuel.Key];
                        sumFuelByHorizontal[fuel.Key] = currentFuel + fuel.Value;
                    }
                }
            }

            var res = sumFuelByHorizontal.Values.Min();
        }


        #region input

        List<long> startingInputList = new List<long> {
